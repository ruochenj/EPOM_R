#' Use ANOVA to select candidate associated regions
...
#'
#' @param cell_type A character vector indicating the cell type of each sample.
#' @param row_select A character vector indicating the row indicies corresponding to a certain state with respect to the original 200 bp matrix.
...
#' @export
anova_select <- function(cell_type, row_select, matrix_gen, state_name = "enhancer", alpha_1 = 1e-10, in_dir = NULL, save = TRUE, out_dir = NULL, cores = detectCores()-1){
  if(is.null(in_dir)){
    in_dir = getwd()
  }
  if(is.null(out_dir)){
  ...
  flag = eval(parse(text =
                      paste("!file.exists('", in_dir, "/mat_anova4", state_name, ".RData')", sep = "")
  ))
  if(flag){
    n = dim(matrix_gen)[1]
    ...
    eval(parse(text =
                 paste("save(l, file = '", out_dir, "/mat_anova4", state_name, ".RData')", sep = "")
    ))
    return(l)
  }else{
    eval(parse(text =
                 paste("load('", in_dir, "/mat_anova4", state_name, ".RData')", sep = "")
    ))
    return(l)
  }
}
